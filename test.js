  ...
  .exec((err, c) => {
    ...
    coinmarketcap.on(config.coin.ticker, (coin) => {
      ...
      captcha.middleware.payout((error, data) => {
        let payoutPerCaptchaHashesXMR = (parseFloat(data.payoutPer1mHashes).toFixed(8) / ( 1000000 / captchaHashes)).toFixed(config.coin.decimals)
        let payoutPerCaptchaHashesUSD = (payoutPerCaptchaHashesXMR * data.xmrToUsd).toFixed(2)
